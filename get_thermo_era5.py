...
def simulate_atb_mol(era, ratio):
    ...
    if ratio:
        ...
        era['alpha355'] = era['beta355']
        era['alpha532'] = era['beta532']
    era = era.sort_index()
...
def interpolate_atb_mol(opar_file, era, ratio):     
    ...
    if ratio:
        new_df.to_pickle(Path("/homedata/nmpnguyen/OPAR/RF/Simul/",lidar.upper(), opar_file.stem.split('-')[0], f"{opar_file.stem}_{ratio}_simul.pkl"))
    else:
        new_df.to_pickle(Path("/homedata/nmpnguyen/OPAR/RF/Simul/",lidar.upper(), opar_file.stem.split('-')[0], f"{opar_file.stem}_simul.pkl"))
...
parser.add_argument("--ratio", "-ratio", type=float, help="Extinction vs Backscatter ratio coef.", required=False)
