...
parser.add_argument("--simulation_opt", "-so", type=str, help="option of simulation molecular dataset, 'r' = 'read'", required=True)
...
for oparpath in opar_folder:
    ...
    if opts.simulation_opt == 'r' : # if READ molecular dataset
        oparsimulpath = Path('/homedata/nmpnguyen/OPAR/RF/Simul', lidars, l+'_simul.pkl')        
        oparsimul = pd.read_pickle(oparsimulpath)
    else: # if cannot READ molecular dataset
